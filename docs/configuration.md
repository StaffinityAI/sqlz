...
path. `.` and `..` are rejected as path components, with one exception: a root
may be spelled exactly `"."`, which means the project root itself — the only way
to scan sources that sit beside `sqlz.ziggy`.
