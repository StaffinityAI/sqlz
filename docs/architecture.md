...
connections, pools, transactions, statements, rows, and driver errors. An
adapter runs unchecked SQL itself — a single statement through the ordinary
operations, a parameterless script through `executeScript` — and exposes the
underlying driver value for features sqlz does not model at all. Adapters do not
attempt to make connection construction identical beyond the shared `std.Io`
argument: every backend is initialized with the application's interface and
retains it, then PostgreSQL adds authentication/network options while SQLite
adds a path, open flags, and typed connection settings.
...
| --- | --- | --- | --- |
| Connection setup | file/URI, open flags, and typed settings (foreign keys, journal mode, synchronous, busy timeout) | network and auth options | `std.Io` supplied at initialization; the rest stays backend-specific |
...
| Raw access | underlying `zqlite` value | underlying `pg` value | explicit `raw()` escape hatch, for driver features rather than for unchecked SQL |
...
Because the tool discovers its own inputs, the build registers every candidate
input file on the check step; otherwise an edited query would report a cache hit
and keep bindings that were never re-checked. See
[build-integration.md](build-integration.md) for the exact rule.
