...
/// Seeds a fresh database with the rows an example needs.
pub fn seed(conn: *sqlz.sqlite.Conn, sql: []const u8) !void {
    try unwrap(conn.executeScript(sql));
}
